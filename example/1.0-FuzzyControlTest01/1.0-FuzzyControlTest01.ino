...
static struct fc_controller fcc;
...
void setup() {
  // put your setup code here, to run once:
	...
	fc_controller_increase_a_input(&fcc, "grease");  // ��֬
	fc_controller_print_input_list(&fcc, "input");
	Serial.printf("\r\n");

	Serial.printf("\r\n");
	fc_controller_add_fuzzy_set_for_input(&fcc, "sludge", "SD", __sludge_SD);
	fc_controller_add_fuzzy_set_for_input(&fcc, "sludge", "MD", __sludge_MD);
	fc_controller_add_fuzzy_set_for_input(&fcc, "sludge", "LD", __sludge_LD);
	fc_controller_print_input_fuzzy_set(&fcc, "sludge", "input-sludge");
	Serial.printf("\r\n");

	Serial.printf("\r\n");
	fc_controller_add_fuzzy_set_for_input(&fcc, "grease", "SG", __grease_SG);
	fc_controller_add_fuzzy_set_for_input(&fcc, "grease", "MG", __grease_MG);
	fc_controller_add_fuzzy_set_for_input(&fcc, "grease", "LG", __grease_LG);
	fc_controller_print_input_fuzzy_set(&fcc, "grease", "input-grease");
	Serial.printf("\r\n");

	Serial.printf("\r\n");
	fc_controller_increase_a_output(&fcc, "time", 0, 60, 1E-3f);  // ϴ��ʱ��
	fc_controller_print_output_list(&fcc, "output");
	Serial.printf("\r\n");

	Serial.printf("\r\n");
	fc_controller_add_fuzzy_set_for_output(&fcc, "time", "VS", __time_VS);
	fc_controller_add_fuzzy_set_for_output(&fcc, "time", "S", __time_S);
	fc_controller_add_fuzzy_set_for_output(&fcc, "time", "M", __time_M);
	fc_controller_add_fuzzy_set_for_output(&fcc, "time", "L", __time_L);
	fc_controller_add_fuzzy_set_for_output(&fcc, "time", "VL", __time_VL);
	fc_controller_print_output_fuzzy_set(&fcc, "time", "output-time");
	Serial.printf("\r\n");
	...
	fc_controller_add_rule(&fcc, "IF sludge-SD AND grease-LG THEN time-L");
	...
	fc_controller_print_rule(&fcc, "rules");
	Serial.printf("\r\n");
	...
	fc_controller_reasoning(&fcc, indata, 2, outdata, 1);
    t = millis() - t;
