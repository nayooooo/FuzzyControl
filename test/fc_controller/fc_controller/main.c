...
HANDLE hMutex;
DWORD startTime;
#define ACCURACY		 1E-1f
#define ACCURACY_STR	"1E-1f"
...
static DWORD WINAPI cal_thread(LPVOID param)
{
	...
	struct fc_controller fcc;
	fc_controller_register(&fcc);

	fc_controller_increase_a_input(&fcc, "sludge");  // 污泥
	fc_controller_increase_a_input(&fcc, "grease");  // 油脂
	...
	fc_controller_increase_a_output(&fcc, "time", 0, 60, ACCURACY);  // 洗衣时间
