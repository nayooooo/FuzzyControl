...
static bool __fc_controler_input_deconstruct_cb(struct fc_input *in)
{
...
static bool __fc_controler_output_deconstruct_cb(struct fc_output *out)
{
...
static bool _fc_controler_input_name_is(list_node node, const char* name)
{
	if (node == nullptr) return false;
	if (node->data == nullptr) return false;

	return !__FC_CONTROLER_STRCMP(((struct fc_input*)(node->data))->name, name);
}

static bool _fc_controler_output_name_is(list_node node, const char* name)
{
	if (node == nullptr) return false;
	if (node->data == nullptr) return false;

	return !__FC_CONTROLER_STRCMP(((struct fc_output*)(node->data))->name, name);
}
...
bool fc_controler_unregister(struct fc_controler* const fcc)
{
	...
	list_delete(fcc->in, (list_node_data_deconstruct_cb)__fc_controler_input_deconstruct_cb);
	list_delete(fcc->out, (list_node_data_deconstruct_cb)__fc_controler_output_deconstruct_cb);
	fcc->in = nullptr;
	fcc->out = nullptr;
	if (fcc->rules != nullptr)
	{
...
bool fc_controler_decrease_a_input(const struct fc_controler* const fcc, const char* name)
{
	...
	if (name == nullptr)
		return list_pop_if(fcc->in, nullptr, list_pred_true, (list_node_data_deconstruct_cb)__fc_controler_input_deconstruct_cb);
	else
		return list_remove_if(fcc->in, (void*)name, (list_pred)_fc_controler_input_name_is, (list_node_data_deconstruct_cb)__fc_controler_input_deconstruct_cb);
}
...
bool fc_controler_delete_all_input(const struct fc_controler* const fcc)
{
	...
	return list_clear(fcc->in, (list_node_data_deconstruct_cb)__fc_controler_input_deconstruct_cb);
}
...
bool fc_controler_decrease_a_output(const struct fc_controler* const fcc, const char* name)
{
	...
	if (name == nullptr)
		return list_pop_if(fcc->out, nullptr, list_pred_true, (list_node_data_deconstruct_cb)__fc_controler_output_deconstruct_cb);
	else
		return list_remove_if(fcc->out, (void*)name, (list_pred)_fc_controler_output_name_is, (list_node_data_deconstruct_cb)__fc_controler_output_deconstruct_cb);
}
...
bool fc_controler_delete_all_output(const struct fc_controler* const fcc)
{
	...
	return list_clear(fcc->out, (list_node_data_deconstruct_cb)__fc_controler_output_deconstruct_cb);
}
...
bool fc_controler_add_fuzzy_set_for_input(const struct fc_controler* const fcc, const char* name, const char* tag, fc_membership_fn ms)
{
	...
	list_node n = list_find_if(fcc->in, (void*)name, (list_pred)_fc_controler_input_name_is);
	if (n == nullptr) return false;
...
bool fc_controler_add_fuzzy_set_for_output(const struct fc_controler* const fcc, const char* name, const char* tag, fc_membership_fn ms)
{
	...
	list_node n = list_find_if(fcc->out, (void*)name, (list_pred)_fc_controler_output_name_is);
	if (n == nullptr) return false;
	if (n->data == nullptr) return false;
...
bool fc_controler_reasoning(const struct fc_controler* const fcc, const struct tag_data* in, fc_size in_num, struct tag_data* out, fc_size out_num)
{
	...
	{
		list_node n = list_find_if(fcc->in, (void*)(in[i].tag), (list_pred)_fc_controler_input_name_is);
	...
	for (fc_size i = 0; i < rule_num; i++)
	{
		...
		while (n != nullptr)
		{
			...
			const char* label = __FC_CONTROLER_STRTOK_S((char*)(cond->cr), "-", &context);  // input name
			list_node inn = list_find_if(fcc->in, (void*)label, (list_pred)_fc_controler_input_name_is);
			if (inn == nullptr || inn->data == nullptr) goto _error_out;
			label = __FC_CONTROLER_STRTOK_S(nullptr, "", &context);  // input tag
		...
		while (n != nullptr)
		{
			...
			char* context = nullptr;
			const char* label = __FC_CONTROLER_STRTOK_S((char*)(cond->cr), "-", &context);  // output name
			list_node outn = list_find_if(fcc->out, (void*)label, (list_pred)_fc_controler_output_name_is);
			if (outn == nullptr || outn->data == nullptr) goto _error_out;
			label = __FC_CONTROLER_STRTOK_S(nullptr, "", &context);  // output tag
	...
	{
		list_node n = list_find_if(fcc->out, (void*)(out[i].tag), (list_pred)_fc_controler_output_name_is);
