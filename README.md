...
模糊输出部件（[fc_output](src/core/fc_output.h)）将激活程度与相应的隶属函数结合，得到`Mamdani`曲线，所有Mamdani曲线取并集得到最终的输出曲线，最后结合实际选择合适的算法得到控制量。

对于部件的使用，有以下几个API。

| 名称 | 参数 | 返回值 | 描述 |
| - | - | - | - |
| fc_output_register | struct fc_output* const,const char*,accurate_number,accurate_number,accurate_number | bool | 注册输出对象，同时设置输出范围和计算步长 |
| fc_output_unregister | struct fc_output* const | bool | 注销输出对象 |
...
$$
val=\frac{\int{xf(x)dx}}{\int{f(x)dx}}
$$
