...
通常一个模态的数据有多个评价标准，每个评价标准又存在多个等级，比如100分的试卷，我们可以认为其中的一种评价标准如下：
...
可见，当分数取到一个允许的特定值时，可以得到带有4个隶属度元素的隶属度向量（模糊子集），比如当分数为80时，隶属度向量为`[0 0.67 0.67 0]`或写成$A=\frac{0}{“优秀”}+\frac{0.67}{“良好”}+\frac{0.67}{“及格”}+\frac{0}{“不及格”}$，这是从“优秀”开始计算的，当然也可以从“不及格”开始排序，显然这个隶属函数是不合适的，为了方便，我就不再修改了。

<font color='red'>注：在编程时，为了降低资源成本，可以使用数组来存储模糊子集，如果需要知道隶属度对应的评价标准，可以使用带有键值对的数据结构进行存储，或者建立一个“伪常量”数组标签，用于存放评价标准（伪常量数组标签用户只读，管理员可写）。</font>
...
$$\mu_{B^{*}}(y)= \max{(\alpha \wedge \mu_{B}(y))}$$

2. 乘积运算规则型
$\\$ 单个规则输出值与激励强度相乘，然后再取所有规则输出值的最大值。
$$\mu_{B^{*}}(y)= \max{(\alpha \cdot \mu_{B}(y))}$$
...
2. 加权平均法（重心法）
$\\$ e.g.
$$b=\frac{\sum\limits_{i=1}^{N} \mu_{B^{*}}(y) \cdot y}{\sum\limits_{i=1}^{N} \mu_{B^{*}}(y)}$$
