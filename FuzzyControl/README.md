...
- [Fuzzy Control（模糊控制）](#fuzzy-control模糊控制)
- [目录](#目录)
- [控制器核心](#控制器核心)
- [相关数学符号说明](#相关数学符号说明)
- [相关概念](#相关概念)
...
模糊集合就是对于特定论域中元素的一个模糊的“划分”，因为它是模糊的，所以我们无法得到一个准确的描述或者值，只能通过隶属函数得到元素隶属于这个模糊集合的程度，即隶属度。就那上述论域所举例子来说，“XXX小区的居民是年轻人”就是这个论域的一个模糊集合，我们无法准确的说这个人就是年轻人，只能说这个人的年轻程度是多少。
...
1. 向量表示法
$\\$ 当论域$X$为有限点集$X=\{x_{1},x_{2},...,x_{n}\}$时，$X$上的模糊集可以用向量$A$来表示，即
$$A=(\mu_{1},\mu_{2},...,\mu_{n}) \quad \mu_{i}=A(x_{i}),i=1,2,...,n$$

2. Zadeh表示法
$\\$ 给定有限论域$X=\{x_{1},x_{2},...,x_{n}\}$，$A$为$X$上的模糊集合
...
$$A=\{(x_{1},A(x_{1})),(x_{2},A(x_{2})),...,(x_{n},A(x_{n}))\}$$
...
$$Q=R \circ S$$
或
$$\mu_{R \circ S}(x,z)= \vee \{\mu_{R}(x,y) \wedge \mu_{S}(y,z)\}$$
...
设有限集$X=\{x_{1},x_{2},...,x_{m}\} \enspace Y=\{y_{1},y_{2},...,y_{n}\}$，$R$是$X \times Y$上的模糊关系，$A$和$B$分别为$X$和$Y$上的模糊集，且满足
$$B=A \circ R$$
